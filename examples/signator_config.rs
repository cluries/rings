...
fn main() {
    ...
    // 1. 基本配置
    let basic_config = SignatorConfig::new(key_loader.clone(), "redis://localhost:6379".to_string())
        .priority(100)
    ...
    // 4. 自定义应用逻辑
    let custom_apply_config = SignatorConfig::new(key_loader.clone(), "redis://localhost:6379".to_string())
        .apply(|parts| {
