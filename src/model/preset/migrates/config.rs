...
fn define_config_table() -> TableCreateStatement {
    Table::create().table(Config::Table).col(
        ColumnDef::new(Config::Id).integer().not_null().auto_increment().primary_key()
    )
        ...
        .col(ColumnDef::new(Config::CreatedAt).date_time().not_null()).to_owned()
}
