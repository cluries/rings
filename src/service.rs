...
impl ServiceManager {
    ...
    fn managed_by_name(&self, name: &str) -> Option<Arc<RwLock<Box<dyn ServiceTrait>>>> {
        self.managed.read().ok()?.iter().find(
            |managed| {
                match managed.try_read() {
                    Err(_) => false,
        ...
        T: ServiceTrait + Default,
    {
        ...
        if self.managed_by_name(&name).is_some() {
            return Err(
                Erx::new(
                    format!("Service '{}' already registered!", name).as_str()
                )
            );
        }
        ...
        match self.managed.try_write() {
            Ok(mut write_guard) => {
                ...
                let warp = Arc::new(
                    RwLock::new(
                        Box::new(ctx) as Box<dyn ServiceTrait>
                    )
                );
    ...
    pub fn unregister<T: ServiceTrait + Default>(&self) -> Result<(), Erx> {
        ...
        self.get::<T>().ok_or(
            Erx::new(
                format!("Service '{}' was not registered!", name).as_str()
            )
        )?.try_write().map_err(
            crate::erx::smp
        )?.release();

        self.managed.try_write().map_err(crate::erx::smp)?
            .retain(
                |m| match m.try_read() {
                    Err(ex) => {
                        tracing::error!("{}", ex);
